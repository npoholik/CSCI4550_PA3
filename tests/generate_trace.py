...
if len(sys.argv) < 5:
    sys.stderr.write("Error: Not Enough Commands. Proper Format:\n python2 genereate_trace.py <Bandwidth (MB/s)> <RTT (msec)> <Elapsed Time (sec)> <Output File Path>")
    sys.exit(1)

# Parse command-line arguments
bandwidth_mbps = float(sys.argv[1])      # e.g., 50 for 50 Mbps, 1 for 1 Mbps
delay_ms = float(sys.argv[2])            # e.g., 10 ms or 200 ms (not used in t>
...
#   1 Mbps = 1,000,000 bits/sec - 1,000,000/1000 = 1000 bits per ms - 1000/8 = >
...
# When enough bytes have accumulated to send a full packet, record the current >
